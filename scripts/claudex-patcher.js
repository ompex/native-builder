...
async function run({ inputs, env = process.env, root, runCommand = captured, toolchainCheck = defaultToolchain, hashFile = hash, emit = (line) => process.stdout.write(`${line}\n`) }) {
  ...
  try {
    ...
    for (const [name, args] of [['mise', ['trust', '--yes', path.join(checkout, 'mise.toml')]], ['mise', ['install', '--locked']], ['mise', ['exec', '--', 'bun', 'install', '--frozen-lockfile']]]) { const result = await command(runCommand, name, args, { cwd: checkout, env: privateEnv }); if (result.code) reject('operation'); }
    ...
    for (const [label, targetInfo] of [['QUALIFIED', input.profile === 'release' ? QUALIFIED : null], ['CURRENT', input.profile === 'ci' ? null : { sha256: input.currentSha256, version: input.currentVersion }]]) if (targetInfo) {
      ...
      if (downloaded.code || !fs.existsSync(targetFile) || hashFile(targetFile) !== targetInfo.sha256) reject('operation'); fs.chmodSync(targetFile, 0o700); targetEnv[`CLAUDEX_PATCHER_${label}_TARGET_EXECUTABLE`] = targetFile; targetEnv[`CLAUDEX_PATCHER_${label}_TARGET_SHA256`] = targetInfo.sha256; targetEnv[`CLAUDEX_PATCHER_${label}_TARGET_VERSION`] = targetInfo.version;
    }
    const gates = []; let driftSummary = null;
    for (const gate of GATES[input.profile]) { const result = await command(runCommand, 'mise', gate.split(' ').slice(1), { cwd: checkout, env: targetEnv }); if (result.code) reject('operation'); gates.push({ name: gate, status: 'passed' }); if (gate === 'mise run verify:release' || gate === 'mise run drift:check') driftSummary = parseDriftSummary(result.stdout); }
    const tracked = path.join(checkout, 'prebuilds/darwin-arm64/claudex-patcher.node'); const rebuilt = path.join(target, 'rebuilt.node');
    if (!fs.existsSync(tracked)) reject('operation'); const rebuiltResult = await command(runCommand, 'mise', ['exec', '--', 'bun', 'scripts/build-native.ts', rebuilt], { cwd: checkout, env: targetEnv });
    if (rebuiltResult.code || !fs.existsSync(rebuilt) || hashFile(tracked) !== hashFile(rebuilt)) reject('operation');
    ...
    emit(error.code === 'input' ? 'PATCHER_INPUT_REJECTED' : error.code === 'toolchain' ? 'PATCHER_TOOLCHAIN_REJECTED' : error.code === 'proof' ? 'PATCHER_PROOF_REJECTED' : 'PATCHER_OPERATION_FAILED'); return { ok: false, code: error.code };
  }
}
